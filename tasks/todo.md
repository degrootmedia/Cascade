...
- `ProductionShot` gains `graphEditGens`/`graphEditGenIndex` (stored edits, newest first),
  ...
  source input), `graphEditSourceRefId` (a reference feeds the source input — single
  source, connecting one displaces the other), and `graphOutputSource` gains `"editgen"`.
- New channel `production:generateEditNode` — one `ipcContract` entry; preload + the drift
...
- New `editgen` node (model select over image-input models, Source hint line, thumbnail
  history strip + ‹n/m› cycle, Generate; labeled Prompt + Source input sockets and the
...
- Wiring: `editprompt→editgen.in-prompt` structural; `imagegen→editgen.in-image` binds
  `graphEditImageSource`, `ref→editgen.in-image` binds `graphEditSourceRefId` (image refs
  only; both `isValidConnection`-gated); `editgen→output` binds `graphOutputSource:
...
- [ ] Manual: edit prompt → Generate lands on the node; pipe editgen→output updates the
...
- **Cursor jump fixed** — typing any prompt box in the node view sent the caret to the
...
- **Resolution dropdown on the edit-image node** — the edit node now has a 1k/2k/4k
...
- Verified: typecheck clean, 80/80 tests, production build passes, `git diff --check` clean.
  ...
  channel signature grew one optional field — still needs a dev restart).
...
INSERTING mid-text jumps to the end — so a per-keystroke caret reset happens on EVERY
...
- My first tests were FALSE PASSES: setting `textarea.value` goes through React's tracked
  value setter, which updates the change-detection tracker — so `onChange` never fired and
  ...
  browser's internal write; the tests now exercise the real `onChange → re-render → caret`
  path.

Three-part fix:
1. **`TriplePrompt` focus-deferral** (`deferExternalWhileFocused`, used by the composer):
   while a box is focused, external value changes are ignored — the user's local edits stay
...
2. **`PromptContentEditor` echo suppression**: the rebuild (which used to drop the caret to
   ...
   last emitted — the DOM already holds it, even if Chromium restructured the contenteditable
...
**Video/edit prompts are now INLINE.** Removed the `videoprompt` and `editprompt` nodes —
